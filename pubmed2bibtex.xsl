...
<xsl:if test="starts-with(ArticleTitle, '[')"><xsl:text>,
    title = {{</xsl:text><xsl:value-of select="concat(substring-before(substring(ArticleTitle, 2), ']'), substring-after(substring(ArticleTitle, 2), ']'))" /><xsl:text>}}</xsl:text></xsl:if>
<xsl:if test="not(starts-with(ArticleTitle, '['))"><xsl:text>,
    title = {{</xsl:text><xsl:value-of select="ArticleTitle" /><xsl:text>}}</xsl:text></xsl:if>
<xsl:apply-templates select="AuthorList" />
<xsl:apply-templates select="Journal" />
<xsl:if test="contains(Pagination/MedlinePgn, '-')"><xsl:text>,
    pages = {</xsl:text><xsl:value-of select="substring-before(Pagination/MedlinePgn, '-')" /><xsl:text>--</xsl:text><xsl:value-of select="substring-after(Pagination/MedlinePgn, '-')" /><xsl:text>}</xsl:text></xsl:if>
<xsl:if test="not(contains(Pagination/MedlinePgn, '-'))"><xsl:text>,
...
<xsl:if test="string-length(JournalIssue/PubDate/MedlineDate) > 0"><xsl:text>,
    year = {</xsl:text><xsl:value-of select="substring-before(JournalIssue/PubDate/MedlineDate, ' ')" /><xsl:text>}</xsl:text></xsl:if>
...
<xsl:if test="string-length(JournalIssue/PubDate/MedlineDate) > 0"><xsl:text>,
    month = {</xsl:text><xsl:value-of select="substring-after(JournalIssue/PubDate/MedlineDate, ' ')" /><xsl:text>}</xsl:text></xsl:if>
</xsl:template>
